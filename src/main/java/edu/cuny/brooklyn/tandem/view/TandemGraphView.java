...
public class TandemGraphView implements Runnable
{
	...
	public TandemGraphView(DistanceList distanceList)
	{
		...
		menuBarView_ = new MenuBarView(frame_, new MenuBarController(
				distanceList, runnable));

		// Controllers
		textRangeSelectorController_ = new TextRangeSelectorController(
				distanceList.getLimitedRange(), runnable);
		graphShifterController_ = new GraphShifterController(
				distanceList.getLimitedRange(), runnable);
		...
		navigatorToolbar_ = new NavigatorToolbar(textRangeSelectorController_,
				graphPanelView_.getGraphicalRangeSelector(), graphShifterView_);
		
		zoomSliderController_ = new ZoomSliderController(
				distanceList.getLimitedRange(), runnable , navigatorToolbar_.getTextRangeSelectorView());
	
		zoomSliderView_ = new ZoomSliderView(zoomSliderController_,
				graphShifterController_);
		}
	...
	public void run()
	{
		...
		navigatorToolbar_.getTextRangeSelectorView().setFields(
				distanceList_.getLimitedRange().getLocal());

		// set minimal size of local range by font size
		int fontWidth = graphPanelView_.getGraphics().getFontMetrics().charWidth(
		'A');
