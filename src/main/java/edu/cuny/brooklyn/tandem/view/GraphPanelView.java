...
//	public final static String SEQUENCE_STRING = "SJKDHFDSLKAJSLKDHFJKLASDHFJKLHHSDJKFGSAGFSDAGFSADJKSAHFJSHAASDLJGHASJKLHFJKASDFHLJKASHDFJKHSADFJKHASJKDHFJLKA";
	...
	public GraphPanelView(DistanceList rl, Runnable runnable)
	{
		...
		graphicalRangeSelector_ = new GraphRangeSelector(this, runnable,
				distances_.getLimitedRange());
		setLayout(new BorderLayout());

		GraphShifterController controller = new GraphShifterController(
				rl.getLimitedRange(), runnable);
	}
...
//		else 
		if (distances_.isEmpty())
		{
			graphRuler_.drawLines(g);
			g.setFont(new Font("Sans Serif", Font.BOLD, 24));
			g.drawString("Please choose an input source.", getWidth() / 3,
					getHeight() / 2);
		}
		else
		{
			// Get the actual graph image and draw it.
			graphImage_ = new TriangleGraphImage(distances_,
					getWidth() - MARGIN, getHeight() - MARGIN);
			Image img = graphImage_.getGraphImage(
					distances_.getLimitedRange().getLocal(),
					distances_.getMaxRepeatSize());
			g.drawImage(img, MARGIN + 1, 0, null);

			// draw the ruler
            g.setColor(Color.gray);
            graphRuler_.drawRuler(g, distances_.getLimitedRange().getLocal(),
                    new Range(0, distances_.getMaxRepeatSize()));
