 ...
 * LimitedRange.java
 *
 * ADT for a limitedRange with a local-range which is always within a global-range
 * Author:
 * Ramin Rakhamimov
 * ramin32@gmail.com
 * http://www.ramrak.net
...
public class LimitedRange
{
	...
	public LimitedRange(Range local, Range global)
			throws IllegalArgumentException
	{
		if (!local.isWithin(global))
			throw new IllegalArgumentException(
					"Local range is not withing global range.");
		global_ = global;
		local_ = local;
	}

	public LimitedRange(int localStart, int localEnd, int globalStart,
			int globalEnd)
	{
	...
	public void setLocal(Range r) throws IllegalArgumentException
	{
		...
		{
			throw new IllegalArgumentException(
					"Range is not within the global range.");
		}

		// Return if localRange about to be set is less than minLocal_
		if (minLocal_ != null)
			if (r.getSize() <= minLocal_)
				throw new IllegalArgumentException(
						"Range is below minimal allowable range!");
