 ...
 * Range.java
 *
 * ADT to represent a range object with start,end,midpoint and size.
 *
 * Author:
 * Ramin Rakhamimov
 * http://www.ramrak.net
...
public class Range implements Comparable<Distance>
{
    ...
    {
        if (getMin() < r.getMin() || getMax() > r.getMax()) return false;
        return true;
    }
    ...
    {
        if (getMin() >= r.getMin() && getMin() < r.getMax()) return true;
        if (getMax() > r.getMin() && getMax() <= r.getMax()) return true;
    ...
    public boolean equals(Object o)
    {
        ...
        if (max_ != range.max_) return false;
        if (size_ != range.size_) return false;
        if (min_ != range.min_) return false;
    ...
    {
        if (getMin() == d.getMin()) return getMax() - d.getMax();
