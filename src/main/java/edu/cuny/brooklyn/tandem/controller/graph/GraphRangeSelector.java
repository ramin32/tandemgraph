/****************************************************
 * ClickBar.java Adds a clicker bar to select ranges to the graph. Author: Ramin
 * Rakhamimov ramin32@gmail.com http://www.ramrak.net
 ***************************************************/
...
public class GraphRangeSelector extends MouseAdapter implements MouseMotionListener
{
    ...
    public void setTextRangeSelectorView(
            TextRangeSelectorView textRangeSelectorView)
    {
    ...
    public void paintComponent(Graphics g, Point point, Dimension dimension)
    {
        ...
        {
            if (endPoint_ == null)
                g.drawLine(startPoint_, barY1_ + 1, startPoint_, barY2_ - 1);
            else
            {
                int min = getMinPoint();
                int max = getMaxPoint();
    ...
    {
        if (e.getY() < barY1_ || e.getY() > barY2_ || e.getX() < barX1_ || e.getX() > barX2_)
            return;
