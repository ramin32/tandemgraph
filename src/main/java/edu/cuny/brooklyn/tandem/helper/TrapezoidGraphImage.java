...
//        if (selectedDistance != null) fillTriangle(graphics, selectedDistance, range.getMin(), xScale, yScale);
    ...
    private void drawTrapezoid(Graphics g, Distance distance, double startPoint, double xScale, double yScale)
    {
        ...
        g.drawPolygon(new int[]{(int) Math.round(x1),(int)  Math.round(x2),(int)  Math.round(x3),(int)  Math.round(x4)},
                      new int[]{(int) Math.round(y1),(int)  Math.round(y2),(int)  Math.round(y3),(int)  Math.round(y4)}, 4);
    }
